...
class PWModulator(gr.sync_block):
    ...
    def _enqueue_from_string(self, message: str) -> np.float32:
        ...
        samples_per_t = int(self._t *  self._Fs)
    ...
    def work(self, input_items, output_items):
        ...
        if not self._is_msg_send:
            if self._curr_idx + len(out) >= len(self._queue):
                left_in_queue = len(self._queue) - self._curr_idx
                out[:left_in_queue] = self._queue[self._curr_idx:]
                self._curr_idx = int(self._t *  self._Fs)
                out[left_in_queue:] = np.zeros(len(out)-left_in_queue, dtype=np.float32)
                self._is_msg_send = True
            else:
